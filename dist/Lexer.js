    ...
    setTokensDesc(tokens) {
        tokens.forEach((tok) => {
            if (tok.type !== TokenType.ENDFILE && tok.type !== TokenType.FINISHLINE) {
                if (tok.errorMsg)
                    this.lexResult += `<p class="error"><span class="line-number">${tok.line}</span>: ${tok.value}: ${tok.errorMsg}</p>`;
                else
                    this.lexResult += `<p><span class="line-number">${tok.line}</span>: ${tok.value}: ${TokenDesc[tok.type]}\n</p>`;
            }
        });
    }
}
